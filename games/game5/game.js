// Addition Game (Game 5) – game logic for simple addition
'use strict';

// Loads local strings and resource definitions
const STRINGS = window.GAME5_STRINGS;
const GAME5_ASSETS = window.TiddeliAssets;

function resolveGame5Asset(path) {
    if (!path) return '';
    if (GAME5_ASSETS && typeof GAME5_ASSETS.resolveGameAsset === 'function') {
        return GAME5_ASSETS.resolveGameAsset('game5', path);
    }
    return path;
}
...
// Constants
const MAX_SUM = 20; // Maximum sum allowed
...
const state = {
    firstNumber: 0, // First number in the addition
    secondNumber: 0, // Second number in the addition
...
function cacheDomElements() {
    titleEl = document.getElementById('game5-title');
    ...
    const gameField = document.getElementById('game5-field');
    if (gameField) {
        const bgPath = 'images/background.png';
        const bgUrl = resolveGame5Asset(bgPath);
...
function startNewRound() {
    // Generate random numbers such that sum <= 20
    // First number: 1 to 10
    state.firstNumber = Math.floor(Math.random() * 10) + 1;
    // Second number: 1 to (20 - firstNumber)
    const maxSecond = Math.min(10, MAX_SUM - state.firstNumber);
    state.secondNumber = Math.floor(Math.random() * maxSecond) + 1;
...
async function startBackgroundAmbience() {
    ...
    try {
        ...
        const resolvedTrack = resolveGame5Asset(trackPath);
